...
namespace WebApi.Services
{
    public class UserService
    {
        ...
        public async Task<string?> LoginAsync(string email, string password)
        {
            ...
            // Adicionamos uma verificação para garantir que PasswordHash não é nulo
            if (string.IsNullOrEmpty(user.PasswordHash) || !_auth.VerifyPassword(password, user.PasswordHash))
            {
                return null;
            }
